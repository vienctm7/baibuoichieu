...
export default function Spendmoney() {
  ...
  return (
    <div>
      <main>
        <p id class="p1">
          Remaining: $217,000,000,000
        </p>
        <p class="p2">You haven't spent a single dollar! start buying!</p>
      </main>
      <div className="contener">
        {products.map((products, index) => (
          <div className="tainghe" key={index}>
            <li>
              <img src={products.image} alt="" />
            </li>
            <div className="price">
              <li className="products">{products.title}</li>
              <li>{products.price}$</li>
            </div>
            <button onClick={() => handleDownTask(index)}>Interest</button>
            <span>{count}</span>
            <button onClick={() => handleAddTask(index)}>Add</button>
          </div>
        ))}
      </div>
